...
void display_max_valued_path_results(tGraphe g, tNumeroSommet start_node){
  int i, nb_nodes;
  ...
  for(i = 0 ; i < nb_nodes ; i++){
    ...
    printf("Longest path from %s to %s = %f \t:", first_name, curr_name, -1*pi[i]);
    display_path(g);
  }
}
...
void max_valued_path(tGraphe g, tNumeroSommet node){
  ...
  while((curr_node_index = get_min_pi()) != ERROR){ /*x as described in handout*/
    ...
    for( i = 0; i < curr_nb_succesor; i++){
      ...
      distance_between_nodes = -1 * grapheRecupValeurArc(g, curr_node_index, curr_succ_index);
...
/****************************DRIVER FOR TESTING********************************/
  ...
  int main(int argc, char ** argv){
    ...
    if (argc<3){
      printf("%s\n","Use as :\t./max_valued_path [grp file name] [start node name]");
      exit(EXIT_FAILURE);
    }

    printf("\nExecuting a tweaked Dijkstra algorithm to obtain maximum valued path, starting from %s.\n", argv[2]);

    graphe = grapheAlloue();
    grapheChargeFichier(graphe,argv[1]);
    max_valued_path(graphe, grapheChercheSommetParNom(graphe, argv[2]));
    display_max_valued_path_results(graphe, grapheChercheSommetParNom(graphe, argv[2]));
    grapheLibere(graphe);
