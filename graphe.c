...
static void lireTokenGaranti(FILE* fic, char token[LONG_MAX_TOKEN]) {
  if (-1 == lireToken(fic, token)) {
    halt("Aucun token n'a pu être lu, fichier non terminé ?");
  };
}
...
static double tokenToDouble(char *token) {
  ...
  res = strtod(token, &p);
  if (*p != '\0') {
    halt("La valeur de l'arc doit être un double : %s trouvé",
	 token);
  }
  return res;
}
...
int grapheChargeFichier(tGraphe graphe, char *fichier) {
  ...
  fic = fopen(fichier, "r");
  if (fic==NULL) {
    halt("Le fichier %s n'a pas pu être ouvert\n", fichier);
  }
  ...
  if (!strcmp(token,TOKEN_ORIENTE)) {
    ...
    halt("Le type du graphe doit être [%s] ou [%s]",
...
void auxVerifieNumeroSommet(tGraphe graphe, 
  ...
  if ( (numero<0) || (numero>=graphe->nbSommets) ) {
    halt("Numéro de sommet incorrect %d\n", numero);
  }
}
...
tNumeroSommet grapheSuccesseurSommetNumero(tGraphe graphe, 
  ...
  if ( (numero<0) || (numero>=nbSuccesseurs) ) {
    halt("Numéro de successeur incorrect (%d) pour le sommet numéro %d", 
	 numero, sommet);
  }
...
tValeurArc grapheRecupValeurArc(tGraphe graphe,
  ...
  if ( (depart<0) || (depart>=graphe->nbSommets) ) {
    halt("Numéro de sommet d'origine incorrect %d\n", depart);
  }

  if ( (arrivee<0) || (arrivee>=graphe->nbSommets) ) {
    halt("Numéro de sommet de destination incorrect %d\n", arrivee);
  }

  if ( !grapheExisteArcEntre(graphe, depart, arrivee) ) {
    halt("Il n'y pas d'arc entre le sommet %d et le sommet %d",
	 depart, arrivee);
  }
