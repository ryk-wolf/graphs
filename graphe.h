 ...
 * - \subpage consignes
 * - \subpage remerciements
 
 \authors François Lemaire, Arnaud Liefooghe, Sylvain Salvati, Marie-Emilie Voge, Léopold Weinberg
 \date 2019
 ...
 * \page presentation Présentation générale
 ...
 En interne, les données sont stockées
...
Les structures de données sont commentées, mais uniquement dans un but
pédagogique. Lorsque vous utilisez la bibliothèque, vous ne devez
<b>jamais</b> accèder directement aux structures (sauf pour la
structure tArc).  Vous devez utiliser les fonctions mises à votre
disposition.

L'intérêt est que si les structures de données de la bibliothèque
changent en cours d'année, votre code continuera de fonctionner. De
plus, lors du contrôle TP de fin de semestre, vous pourriez avoir une
version de la librairie avec des structures de données différentes, il
vaut mieux donc apprendre à utiliser uniquement les interfaces de
fonctions.
...
\page remerciements Remerciements

Merci aux personnes suivantes qui ont aidé à trouver
de (rares !) bogues dans la librairie : Bilal Berrehail, Abdelhakim Said.
 */
 ...
 *
 * Définit les opérations d'allocation, de chargement et de 
...
/** Crée un graphe aléatoire 
    ...
    \param estOriente : 1 si on veut un graphe orienté, 0 sinon
 ...
 *
 * Définit quelques opérations de modification d'un graphe.
 */
/*@{*/
/**
 ...
 *
 * Définit les opérations de consultation de graphes.
 */
/*@{*/
...
/**
   ...
   \param graphe : un graphe \b orienté
   \param sommet : un numéro de sommet
   \param i : un numéro de prédécesseur

   Renvoie le prédécesseur numéro \p i
   du sommet \p sommet.
   \warning déclenche une erreur si le graphe n'est pas orienté
 ...
 *
 * Définit une opération de déclenchement d'erreur.
 *
